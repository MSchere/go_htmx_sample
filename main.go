...
func (t *TODO) HTML(r *http.Request) string {
	tmpl := template.Must(template.New("todo").Parse(todoRowTemplate))
...
// Setup redis storage if environment variables are defined
...
func main() {
	...
	// HTMX requires a custom 200 response code for DELETE requests
	api.SetCustomResponseCode(http.MethodDelete, http.StatusOK)
	// Initialize storage (Redis) if environment variables are defined, otherwise use in-memory storage
